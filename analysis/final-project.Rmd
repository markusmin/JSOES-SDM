...
BC_coast <- sf::st_crop(BC_spdf,
    c(xmin = -126, ymin = 44, xmax = -123, ymax = 48.5))


# convert both shapefiles to a different projection (UTM zone 10) so that they can be plotted with the sdmTMB output
US_west_coast_proj <- sf::st_transform(US_west_coast, crs = 32610)
BC_coast_proj <- sf::st_transform(BC_coast, crs = 32610)
...
grid <- st_make_grid( st_bbox(c(xmin=-126, xmax=-123, ymin=44, ymax=48.5)), n=c(10,40), crs = "WGS84")
grid <- sf::st_transform(grid, crs = 32610)
strait <- st_make_grid( st_bbox(c(xmin=-124.5, xmax=-123, ymin=47.9375, ymax=48.5)), n=c(5,5), crs = "WGS84")
strait <- sf::st_transform(strait, crs = 32610)
st_difference(grid, strait) -> survey_grid # remove strait of Juan de Fuca
...
```{r predict-to-grid}
# create base map
survey_area_basemap <- ggplot(US_west_coast_proj) +
  geom_sf() +
...
coast_proj <- sf::st_transform(US_west_coast_proj$geometry, crs = 32610)
...
```{r calculate_dist_shore}
grid_yrs_sf <- st_as_sf(grid_yrs, crs = 32610, coords = c("X", "Y"))

st_distance(grid_yrs_sf, US_west_coast_proj) -> grid_dist_shore
# this is currently way off because of units I think - I think US_west_coast_proj is in m and grid_yrs_sf is in km

```


```{r reformat-SST}
SST1 <- read.csv(here::here("Data", "noaacrwsstDaily_1b80_3259_2114.csv"), skip = 1)
SST2 <- read.csv(here::here("Data", "noaacrwsstDaily_3b63_608b_d6fb.csv"), skip = 1)
SST3 <- read.csv(here::here("Data", "noaacrwsstDaily_3c79_0cf9_15fe.csv"), skip = 1)
SST4 <- read.csv(here::here("Data", "noaacrwsstDaily_9e43_d7a8_58eb.csv"), skip = 1)
SST5 <- read.csv(here::here("Data", "noaacrwsstDaily_97b1_1c15_1964.csv"), skip = 1)
SST6 <- read.csv(here::here("Data", "noaacrwsstDaily_0802_800d_255f.csv"), skip = 1)
SST7 <- read.csv(here::here("Data", "noaacrwsstDaily_7783_2164_b474.csv"), skip = 1)

SST1 %>% 
  bind_rows(., SST2, SST3, SST3, SST5, SST6, SST7) -> SST
...
Data = list( "D_i"= csyis$n_per_km, "t_i" = csyis$year - min(csyis$year), 
             "temp_i" = csyis$x3m_temp_c, "dist_i" = csyis$km_from_shore,
             "n_t" = length(unique(csyis$year)),
             "A_is"=A_is, "A_gs"=A_gs, "M0"=spde$c0, "M1"=spde$g1, "M2"=spde$g2 )
Params = list( "beta_t"=rep(0, length(unique(csyis$year))), 
               "ln_tau_omega"=0, "ln_tau_epsilon"=0,
