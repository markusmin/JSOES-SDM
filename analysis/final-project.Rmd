...
collocation_metric <- function(grid1, grid2){
  ...
  # get the total number of predators and prey
  total_est1 <- sum(combined_grid$est1)
  total_est2 <- sum(combined_grid$est2)
  
  # get proportions of the total number
  combined_grid %>% 
    mutate(p_est1 = est1/total_est1) %>% 
    mutate(p_est2 = est2/total_est2) -> combined_grid
  
  combined_grid %>% 
    mutate(predxprey = p_est1 * p_est2) -> combined_grid
  ...
  combined_grid %>% 
    group_by(year) %>% 
    summarise(denominator = sqrt(sum(p_est1^2)*sum(p_est2^2))) -> denominator_by_year
