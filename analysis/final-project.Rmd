...
csyis <- filter(csyis, !(is.na(x3m_temp_c)))
...
```{r start creation prediction grid}
...
# grid_yrs_sf <- st_as_sf(grid_yrs, crs = UTM_zone_10_crs, coords = c("X", "Y"))

# make this projection into kilometers
# US_west_coast_proj_km <- st_as_sf(US_west_coast_proj$geometry/1000, crs = UTM_zone_10_crs)

# st_distance(grid_yrs_sf, US_west_coast_proj_km) -> grid_dist_shore
...
```{r create_prediction_grid}
# some sanity checks to make sure that things are how they should be

# are all points within 100 km from shore?
# yeah, pretty much, maybe a few over that we'll consider just rounding errors
range(survey_domain_cov$sf_dist_shore)
...
# Fit sdmTMB with effect of year and effect of space and spatio-temporal effect
jsoes_st_cov_sdmTMB <- sdmTMB(n_per_km ~ 0 + as.factor(year) + SST_scaled + dist_shore_scaled,
                         data = csyis,
                         mesh = mesh_sdmTMB,
                         time = "year",
                         spatial = "on",
...
sanity(jsoes_st_cov_sdmTMB)
AIC(jsoes_st_cov_sdmTMB)
...
jsoes_st_cov_sdmTMB_pred <- predict(jsoes_st_cov_sdmTMB, newdata = survey_predict_grid)
...
jsoes_density_predictions_cov <- survey_area_basemap_km +
  geom_tile(data = jsoes_st_cov_sdmTMB_pred, aes(x = X, y = Y, fill = exp(est)),
            width = 7, height = 7) +
  scale_fill_viridis_c() +
  facet_wrap(~year) +
  theme_light()

ggsave(here::here("figures", "jsoes_st_cov_sdmTMB_pred_plots.png"), plot = jsoes_density_predictions_cov, height = 30, width = 25)
```
