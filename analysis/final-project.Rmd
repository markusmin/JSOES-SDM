...
ggsave(here::here("figures", "SST_plots.png"), plot = SST_plots, height = 20, width = 15)
```
...
plot_distribution <- function(data){
  ...
  survey_area_basemap +
    ...
    ggtitle(paste0(unique(data$species))) -> species_distribution_plot
...
# grid <- st_make_grid( st_bbox(c(xmin=-126, xmax=-123, ymin=44, ymax=48.5)), n=c(10,40), crs = "WGS84")
# grid <- sf::st_transform(grid, crs = UTM_zone_10_crs)
# strait <- st_make_grid( st_bbox(c(xmin=-124.5, xmax=-123, ymin=47.9375, ymax=48.5)), n=c(5,5), crs = "WGS84")
# strait <- sf::st_transform(strait, crs = UTM_zone_10_crs)
# st_difference(grid, strait) -> survey_grid # remove strait of Juan de Fuca
# st_difference(survey_grid, US_west_coast_proj) -> survey_grid # remove land
# st_as_sf(survey_grid) -> survey_grid_df # convert sfc_geometry to data.frame
...
Based on our model selection process for the selection of spatial and temporal effects, we have now added covariates into our best model (spatial effect, spatiotemporal random fields that are IID)s.
...
compile(here::here("analysis", "jsoes_st_cov_sdm_1.cpp"))
dyn.load( dynlib(here::here("analysis", "jsoes_st_cov_sdm_1")))
...
csyis_st_cov_sdmTMB <- sdmTMB(n_per_km ~ 0 + as.factor(year) + SST_scaled + dist_shore_scaled,
                         data = csyis,
                         mesh = mesh_sdmTMB,
                         time = "year",
                         spatial = "on",
...
sanity(csyis_st_cov_sdmTMB)
AIC(csyis_st_cov_sdmTMB)



### COHO YEARLINGS
coho_st_cov_sdmTMB <- sdmTMB(n_per_km ~ 0 + as.factor(year) + SST_scaled + dist_shore_scaled,
                         data = coho,
                         mesh = mesh_sdmTMB,
                         time = "year",
                         spatial = "on",
...
sanity(coho_st_cov_sdmTMB)
AIC(coho_st_cov_sdmTMB)



### WATER JELLIES
water_jellies_st_cov_sdmTMB <- sdmTMB(n_per_km ~ 0 + as.factor(year) + SST_scaled + dist_shore_scaled,
                         data = water_jellies,
                         mesh = mesh_sdmTMB,
                         time = "year",
                         spatial = "on",
...
sanity(water_jellies_st_cov_sdmTMB)
AIC(water_jellies_st_cov_sdmTMB)

# check for alignment
# Not the same. I truly have no idea why...
csyis_st_cov_sdmTMB
st_cov_report$beta_t
st_cov_report$beta_dist
st_cov_report$beta_temp

# Maybe we can see a difference in number of parameters?
csyis_st_cov_sdmTMB$tmb_params


# Fit another model, with splines for depth and distance
jsoes_st_cov_splines_sdmTMB <- sdmTMB(n_per_km ~ 0 + as.factor(year) + s(SST_scaled) + s(dist_shore_scaled),
                         data = csyis,
                         mesh = mesh_sdmTMB,
                         time = "year",
                         spatial = "on",
...
sanity(jsoes_st_cov_splines_sdmTMB)
AIC(jsoes_st_cov_splines_sdmTMB)
...
csyis_st_cov_sdmTMB_pred <- predict(csyis_st_cov_sdmTMB, newdata = survey_predict_grid)
coho_st_cov_sdmTMB_pred <- predict(coho_st_cov_sdmTMB, newdata = survey_predict_grid)
water_jellies_st_cov_sdmTMB_pred <- predict(water_jellies_st_cov_sdmTMB, newdata = survey_predict_grid)
...
csyis_prediction_map <- plot_density_predictions(predictions = csyis_st_cov_sdmTMB_pred)
coho_prediction_map <- plot_density_predictions(predictions = coho_st_cov_sdmTMB_pred)
water_jellies_prediction_map <- plot_density_predictions(predictions = water_jellies_st_cov_sdmTMB_pred)
...
```{r covariate_model_diagnostics}
csyis$cov_resids <- residuals(csyis_st_cov_sdmTMB)
qqnorm(csyis$cov_resids)
qqline(csyis$cov_resids)

csyis_st_cov_sdmTMB_residuals_plot <- ggplot(csyis, aes(X, Y, col = cov_resids)) +
  scale_colour_gradient2() +
  geom_point() +
  facet_wrap(~year) +
  coord_fixed()

ggsave(here::here("figures", "csyis_st_cov_sdmTMB_residuals_plots.png"), plot = csyis_st_cov_sdmTMB_residuals_plot, height = 30, width = 25)
```
...
csyis_st_cov_sdmTMB_pred_TMB <- predict(csyis_st_cov_sdmTMB, newdata = survey_predict_grid, return_tmb_object = TRUE)
coho_st_cov_sdmTMB_pred_TMB <- predict(coho_st_cov_sdmTMB, newdata = survey_predict_grid, return_tmb_object = TRUE)
water_jellies_st_cov_sdmTMB_pred_TMB <- predict(water_jellies_st_cov_sdmTMB, newdata = survey_predict_grid, return_tmb_object = TRUE)

csyis_index_of_abundance <- get_index(csyis_st_cov_sdmTMB_pred_TMB, area = 24, bias_correct = TRUE)
coho_index_of_abundance <- get_index(coho_st_cov_sdmTMB_pred_TMB, area = 24, bias_correct = TRUE)
water_jellies_index_of_abundance <- get_index(water_jellies_st_cov_sdmTMB_pred_TMB, area = 24, bias_correct = TRUE)
...
competition_index <- collocation_metric(grid1 = csyis_st_cov_sdmTMB_pred, grid2 = coho_st_cov_sdmTMB_pred)
predator_prey_index <- collocation_metric(grid1 = csyis_st_cov_sdmTMB_pred, grid2 = water_jellies_st_cov_sdmTMB_pred)
```
