...
jsoes_null_sdmTMB <- sdmTMB(n_per_km ~ 1,
                         data = csyis,
                         mesh = mesh_sdmTMB,
                         time = "year",
                         spatial = "on",
...
pred.jsoes_null_sdmTMB = predict(jsoes_null_sdmTMB, newdata = predict_grid_1998, return_tmb_object = FALSE)

# This is a workaround to get it into the right format which I think (?) works...?
grid_density_sdmTMB = st_sf(st_as_sfc(survey_grid_df), est=pred.jsoes_null_sdmTMB$est, crs=st_crs(survey_grid_df) )

ggplot(grid_density_sdmTMB) +
  geom_sf(aes(fill = est))

# compare with the bespoke TMB implementation
ggplot(grid_density) +
  geom_sf(aes(fill = ln_d_gt.1))
