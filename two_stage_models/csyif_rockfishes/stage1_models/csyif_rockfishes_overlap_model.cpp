...
for( int t=0; t<n_t; t++){
  for( int g=0; g<n_g_csyif; g++){
    ov_gt_csyif_rf(g,t) = d_gt_csyif(g,t) * d_gt_rf(g,t); // calculate numerator for each cell
    scaled_dens_gt_csyif(g,t) = d_gt_rf(g,t); // calculate the denominator for each cell
  }
  
  // calculate the index for each year
  ov_csyif_rf_t(t) = ov_gt_csyif_rf.col(t).sum()/scaled_dens_gt_csyif.col(t).sum();
}

// Reporting: overlap
REPORT(ov_csyif_rf_t); // report overlap for full JSOES domain
ADREPORT(ov_csyif_rf_t); // report overlap for full JSOES domain

// REPORT(ov_csyif_rf_t); // report overlap for only coverage overlap between JSOES and PRS
// ADREPORT(ov_csyif_rf_t); // report overlap for only coverage overlap between JSOES and PRS
