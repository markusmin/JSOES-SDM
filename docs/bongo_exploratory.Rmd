...
Based on the Moran's I results, we see that there is evidence for spatial clustering in some years than others. However, there is not a clear slope to the semivariogram, which indicates that the degree of spatial autocorrelation at our sampling variation and that there is a lot of noise in our data. It suggests that copepods are quite patchy, with these patches occurring on a scale less than the scale of the sampling resolution (which is 5 nm along a single transect orthogonal to the coast).
