...
The JSOES bongo data does not catch many direct prey items of Pacific salmon, but it does catch the prey items of the prey of Pacific salmon. The direct salmon prey that is caught in this survey includes Euphausiids, Cancer crab larvae, hyperiid amphipods, and non-cancer crab larvae. The abundance of copepods has also been correlated with salmon marine survival, and as such we will also examine these taxa.

Let's ID everything that's seen in at least 1% of samples.
...
bongo_taxon_groups %>% 
  mutate(group = ifelse(grepl("shrimp", group), "shrimp larvae", group)) -> bongo_taxon_groups
...
I'm not sure whether these copepod taxa would be classified as cold-water or warm-water - they aren't mentioned in the publications on warm vs. cold water assemblages.

## Plotting distributions of some common taxa

Let's export the following groups for our study, since they are known direct prey items of salmon:
- Cancer crab larvae
...
```{r create_station_grid, eval = FALSE}
...
```{r coastwide_temporal_autocorrelation, eval = FALSE}
...
```{r spatial_autocorrelation_functions, eval = FALSE}
...
```{r spatial_autocorrelation_cama, fig.show = "hold", out.width = "25%", eval = FALSE}
### Calanus marshallae
...
```{r spatial_autocorrelation_cama_moran_I, eval = FALSE}
...
```{r spatial_autocorrelation_capa, fig.show = "hold", out.width = "25%", eval = FALSE}
### Calanus pacificus
# Figure out what years have any catch of this taxon
bongo_catch_LH_collapse %>% 
...
```{r spatial_autocorrelation_capa_moran_I, eval = FALSE}
# Calculate Moran's I for each
