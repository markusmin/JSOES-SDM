...
# lat and lon are in hundredths of degrees, and long is not negative, so convert
PWCC_MWT_haul %>% 
  mutate(net_fishing_lat = net_fishing_lat/100) %>% 
  mutate(net_fishing_long = -1 * net_fishing_long/100) -> PWCC_MWT_haul
...
# turn this into an sf object
st_as_sf(PWCC, coords = c("net_fishing_long", "net_fishing_lat"), crs = 4326) -> PWCC_sf
...
PRS_density_sums[1:10,] %>% 
  ...
  kbl(caption = "PRS: Top ten taxa by mean density.") %>% 
...
PRS_freq_occur[1:10, ] %>% 
  ...
  kbl(caption = "PRS: Top ten taxa by frequency of occurrence.") %>% 
  kable_styling(latex_options = "HOLD_position")
...
PWCC_density_sums[1:10,] %>% 
  mutate(mean_number = round(mean_number, 1),
         sd_number = round(sd_number, 1)) %>% 
  kbl(caption = "PWCC: Top ten taxa by mean density.") %>% 
  kable_styling(latex_options = "HOLD_position")
...
PWCC_freq_occur[1:10, ] %>% 
  mutate(prop_samples = round(prop_samples, 2)) %>% 
  kbl(caption = "PWCC: Top ten taxa by frequency of occurrence.") %>% 
  kable_styling(latex_options = "HOLD_position")
...
ggplot(US_west_coast) +
  ...
  # add PWCC stations
  geom_point(data = PWCC, aes(x = net_fishing_long, y = net_fishing_lat), color = "red")
...
sanddab_dist_plot <- plot_distribution(data = PRS, taxon_name = "Citharichthys sordidus")

sanddab_dist_plot

euphausiidae_dist_plot <- plot_distribution(data = PRS, taxon_name = "Euphausiidae")
...
survey_area_basemap +
      geom_point(data = group_data, aes(x = net_fishing_long, y = net_fishing_lat, size = total, color = encounter),
                 alpha = 0.5) +
    scale_color_manual(values = c("zero" = "#fc9272", "non-zero" = "#2ca25f")) +
    facet_wrap(~year, nrow = 2) +
    ggtitle(group_name) +
