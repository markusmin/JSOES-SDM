...
bongo_catch %>% 
  ...
  left_join(dplyr::select(bongo_freq_occ, genus_species, prop_samples), by = "genus_species") -> bongo_catch_summary

bongo_catch_summary %>% 
  left_join(bongo_taxon_groups, by = "genus_species") %>% 
  group_by(group) %>% 
  summarise(group_total = sum(total)) -> bongo_group_catch_summary

bongo_catch_summary %>% 
  left_join(bongo_taxon_groups, by = "genus_species") %>% 
  left_join(bongo_group_catch_summary, by = "group") %>% 
  mutate(prop_group_total = total/group_total) -> bongo_catch_summary
...
bongo_catch_summary %>% 
  ...
  filter(!is.na(life_history_stages)) -> bongo_catch_summary

write.csv(bongo_catch_summary, here::here("figures", "paper_figures", "JSOES_bongo_functional_groups_summary.csv"))
